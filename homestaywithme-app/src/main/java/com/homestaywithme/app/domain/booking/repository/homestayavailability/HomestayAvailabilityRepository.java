package com.homestaywithme.app.domain.booking.repository.homestayavailability;
...
public interface HomestayAvailabilityRepository extends JpaRepository<HomestayAvailability, HomestayAvailabilityId> {
    ...
    @Query("""
            SELECT new com.homestaywithme.app.domain.booking.repository.homestayavailability.model.SetPriceHomestayAvailability(h.id, CAST(AVG(ha.price) AS BigDecimal), MAX(ha.date))
            FROM Homestay h
